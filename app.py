...
def add_working_time(start_dt, duration_mins):
    ...
    while remaining > 0:
        if current_time.year > 2100: break
...
def process_data(master_file, delivery_file, setup_file, track_file, recv_file, target_date):
    ...
    if recv_file:
        ...
        r_mcs = find_col(recv_df, ['MCS#'], default_idx=7)
        r_date = find_col(recv_df, ['RECV', 'DATE'], default_idx=1)
        if r_mcs and r_date:
            for _, r in recv_df.iterrows():
                try: 
                    dt = pd.to_datetime(r[r_date], errors='coerce')
                    if pd.notna(dt) and dt.year < 2100: recv_schedule[str(r[r_mcs]).strip()] = dt
                except: pass
    ...
    for _, d_row in delivery_df.dropna(subset=[mcs_col_d]).iterrows():
        ...
        qty = pd.to_numeric(d_row.get(qty_col, 0), errors='coerce')
        if pd.isna(qty) or qty <= 0: continue
        mach = get_internal_routing(mcs)
        if not mach: continue
        
        recv_dt = recv_schedule.get(mcs, None)
        recv_str = recv_dt.strftime('%m/%d') if pd.notna(recv_dt) else "確認中"
        ...
        jobs.append({'優先度': rank, '機械': mach, 'MCS#': mcs, '出荷日': str(d_row.get(due_col, '-')), '数量': int(qty), '入荷予定': recv_str, 'recv_dt': recv_dt})
...
if f_master and f_delivery and f_setup:
    ...
    # 負荷グラフ（時間単位）
    st.markdown("### 📊 本日の負荷状況 (単位: 時間)")
    target_df = raw_df[raw_df['優先度'].str.contains('A|B', na=False)]
    workload_data = [{'機械': m, '稼働予定(時間)': round((len(target_df[target_df['機械']==m])*25 + target_df[target_df['機械']==m]['数量'].sum()/100*60)/60, 1)} for m in machine_list]
    st.altair_chart(alt.Chart(pd.DataFrame(workload_data)).mark_bar().encode(x='稼働予定(時間):Q', y=alt.Y('機械:N', sort='-x'), color=alt.condition(alt.datum['稼働予定(時間)'] > 8, alt.value('#e74c3c'), alt.value('#3498db'))).properties(height=200), use_container_width=True)
    ...
    st.download_button(
        label="📥 確定したスケジュールをダウンロード (Excel対応版)",
        data=csv_buffer.getvalue(),
        file_name=f'production_plan_{target_date.strftime("%Y%m%d")}.csv',
        mime='text/csv'
    )
